...
document.addEventListener("DOMContentLoaded", async () => {
  if (window.location.pathname.includes("menu-beverage.html")) {
    await fetchMenu();
  }
});
...
const fetchMenu = async () => {
  try {
    const response = await fetch(
      `https://be-2-bandung-19-production.up.railway.app/products/3`
    );
...
const displayMenu = (menu) => {
  const section = document.querySelector(".menu-drink");
  section.innerHTML = "";
