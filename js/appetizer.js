...
document.addEventListener("DOMContentLoaded", async () => {
  if (window.location.pathname.includes("menu-appetizer.html")) {
    await fetchMenu();
  }
});
...
const fetchMenu = async () => {
  try {
    const response = await fetch(`https://be-2-bandung-19-production.up.railway.app/products/1`);
...
const displayMenu = (menu) => {
  const section = document.querySelector(".menu-appetizer");
  section.innerHTML = "";
  ...
  menu.forEach((product) => {
    ...
    div.innerHTML = `
                ...
                <h4 class="product-price"> Rp.${product.price.toLocaleString("id-ID")}  </h4>
                <div class="menu-button">
                    <img class="remove-order" style="float: left;" src="assets/minus-circle.svg"
                    onclick="removeFromcart('${product.name}', ${product.price})" />
                    <img class="add-order" style="float: right;" src="assets/plus-circle.svg" onclick="addToCart('${product.name}', ${product.price})" />
                </div>
        `;
